...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
        		<div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">待审核评价列表</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
                            				...
                            				<table width="100%" class="table table-striped table-bordered table-hover dataTable no-footer dtr-inline" id="dataTables" role="grid" aria-describedby="dataTables-example_info" style="width: 100%;">
                                				...
                                				<tbody>
                                					@forelse ($comData as $comData)
				                                        ...
				                                        <td>{{$comData->act_name}} <a href="/actshowprev?id={{$comData->act_id}}" target="_blank">预览</a></td>
				                                        <td>{{$comData->user_email}}</td>
				                                        <td>{{$comData->star_val}}</td>
				                                        <td>{{$comData->com_content}}</td>
				                                        <td>{{$comData->com_create}}</td>
				                                        <td>
			                                        			<a href="javascript:void(0)" onclick="confirm('确认通过？')?location.href='/admin/comment/mod_com?id={{$comData->id}}&stas=1':'';">通过</a> <a href="javascript:void(0)" onclick="confirm('确认删除？')?location.href='/admin/comment/mod_com?id={{$comData->id}}&stas=0':'';">删除</a>		                                        		
				                                        </td>
	                           						</tr>												    
												@empty
												    <p>没有数据</p>
